...
public class UsuarioTest {
    ...
    public void testGetAgenda() {
        ...
        assertEquals(0, usuario.getAgenda().getContactos().size(), "La agenda debería estar vacía inicialmente.");
    }
    ...
    public void testBuscarContacto() {
        ...
        Contacto contacto = new Contacto("Pedro", "192.168.1.2", 8081);
        usuario.agregarContacto(contacto);
        Contacto contactoExistente = usuario.getAgenda().obtenerContactoPorIpYPuerto("192.168.1.2", 8081);
        ...
        Contacto contactoInexistente = usuario.getAgenda().obtenerContactoPorIpYPuerto("192.168.1.3", 8082);
        assertNull(contactoInexistente, "El contacto no debería existir en la agenda.");

        // Caso 3: IP inválida (parámetro no válido)
        
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto(null, 8081), "No debería encontrar contacto con una IP null.");
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto("999.999.999.999", 8081), "No debería encontrar contacto con una IP con formato inválido.");
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto("dasdasd", 8081), "No debería encontrar contacto con una IP con letras.");
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto("", 8081), "No debería encontrar contacto con una IP vacía.");
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto("198.168.0", 8081), "No debería encontrar contacto con una IP incompleta.");

        // Caso 4: Puerto inválido (parámetro no válido)
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto("198.168.1.2", 70000), "No debería encontrar contacto con un puerto inválido.");
        assertNull(usuario.getAgenda().obtenerContactoPorIpYPuerto("198.168.1.2", -1), "No debería encontrar contacto con un puerto inválido.");
    }

    @Test
    public void testAgregarContacto() {
        // Caso 1: Agregar un contacto válido
        Contacto nuevo = new Contacto("Lucas", "192.168.1.4", 8083);
        usuario.getAgenda().agregarContacto(nuevo);
        assertNotNull(nuevo, "El contacto debería agregarse correctamente.");
        assertEquals(1, usuario.getAgenda().getContactos().size(), "Debería haber 1 contacto en la agenda.");
        ...
        Contacto contactoDiferentePuerto = new Contacto("Juan", "192.168.1.4", 8084);
        usuario.getAgenda().agregarContacto(contactoDiferentePuerto);
        ...
        assertEquals(2, usuario.getAgenda().getContactos().size(), "Debería haber 2 contactos en la agenda.");

        // Caso 3: Agregar un contacto con la misma IP y mismo puerto (debería fallar)
        Contacto contactoDuplicado = new Contacto("Carlos", "192.168.1.4", 8083);
        usuario.getAgenda().agregarContacto(contactoDuplicado);
        ...
        Contacto aux = new Contacto(null, "192.168.1.5", 8085);
        assertTrue(usuario.agregarContacto(aux), "No debería permitir agregar un contacto con una IP null.");
        aux = new Contacto("", "192.168.1.5", 8085);
        assertTrue(usuario.agregarContacto(aux), "No debería permitir agregar un contacto con una IP con formato inválido.");
        aux = new Contacto("NombreDemasiadoLargoParaValidacion", "192.168.1.5", 8085);
        assertTrue(usuario.agregarContacto(aux), "No debería permitir agregar un contacto con una IP con letras.");
        
        // Caso 5: Agregar un contacto con IP inválida (parámetro no válido)
        Contacto aux1 = new Contacto("Carlos", null, 8084);
        Contacto aux2 = new Contacto("Carlos", "999.999.999.999", 8084);
        Contacto aux3 = new Contacto("Carlos", "dasdasd", 8084);
        Contacto aux4 = new Contacto("Carlos", "", 8084);
        Contacto aux5 = new Contacto("Carlos", "192.168.0", 8084);
        assertTrue(usuario.agregarContacto(aux1), "No debería permitir agregar un contacto con una IP null.");
        assertTrue(usuario.agregarContacto(aux2), "No debería permitir agregar un contacto con una IP con formato inválido.");
        assertTrue(usuario.agregarContacto(aux3), "No debería permitir agregar un contacto con una IP con letras.");
        assertTrue(usuario.agregarContacto(aux4), "No debería permitir agregar un contacto con una IP vacía.");
        assertTrue(usuario.agregarContacto(aux5), "No debería permitir agregar un contacto con una IP incompleta.");

        // Caso 6: Agregar un contacto con puerto inválido (parámetro no válido)
        aux1 = new Contacto("Carlos", "192.168.1.5", 70000);
        aux2 = new Contacto("Carlos", "192.168.1.5", -1);
        assertNull(usuario.agregarContacto(aux1), "No debería permitir agregar un contacto con un puerto inválido.");
        assertNull(usuario.agregarContacto(aux2), "No debería permitir agregar un contacto con un puerto inválido.");
    }

    @Test
    public void testBuscarConversacion() {
        // Caso 1: La conversación existe
        Contacto aux1 = new Contacto("Ana", "192.168.1.6", 8086);
        usuario.agregarContacto(aux1);
        Conversacion aux2= new Conversacion(aux1);
        usuario.agregarConversacion(aux2);
    ...
    public void testAgregarConversacion() {
        ...
        Contacto aux = new Contacto("Carlos", "192.168.1.7", 8087);
        usuario.agregarContacto(aux);
        Conversacion nueva = new Conversacion(aux);
        usuario.agregarConversacion(nueva);
